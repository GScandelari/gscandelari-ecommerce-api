...
// `verifyInternalToken`, duplicado em Payments (Decisao tecnica 4). Protege
// `POST /internal/payment-intents` (Task 9.2.1) - RN18.
...
describe("Middleware verifyInternalToken (em Payments, recebe chamadas de Orders) - RN18 (Task 9.1.2 / 12.2.3)", () => {
  beforeEach(() => {
  ...
  it("RN18: email do chamador fora da allow-list -> 401", async () => {
    mockVerifyIdToken.mockResolvedValue({
  ...
  it("RN18: token valido, aud correto e email na allow-list -> segue adiante (next), sem 401", async () => {
    ...
    const { req, res, next } = mockReqRes({ authorization: "Bearer token-valido-de-orders" });
